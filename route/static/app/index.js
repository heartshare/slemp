...
function getLoad(data) {
    $("#LoadList .mask").html("<span id='Load' style='font-size:14px'>Getting..</span>");
        ...
        for (var i = 0; i < rdata.length; i++) {
            if (rdata[i].path == '/' || rdata[i].path == '/www') {
                if (rdata[i].size[2].indexOf('M') != -1) {
                    $("#messageError").show();
                    $("#messageError").append('<p><span class="glyphicon glyphicon-alert" style="color: #ff4040; margin-right: 10px;"></span> ' + lan.get('diskinfo_span_1', [rdata[i].path]) + '<a class="btlink" href="javascript:ClearSystem();">[bersihkan sampah]</a></p>');
                }
                var ipre = parseInt(rdata[i].inodes[3].replace('%', ''));
                if (ipre > 95) {
                    $("#messageError").show();
                    $("#messageError").append('<p><span class="glyphicon glyphicon-alert" style="color: #ff4040; margin-right: 10px;"></span>partisi[' + rdata[i].path + ']Penggunaan node' + ipre + '%, ketika tingkat penggunaan mencapai 100%, file tidak dapat dibuat di partisi ini, harap bersihkan tepat waktu!<a class="btlink" href="javascript:ClearSystem();">[bersihkan sampah]</a></p>');
                }
            }
            ...
            dBody = '<li class="col-xs-6 col-sm-3 col-md-3 col-lg-2 mtb20 circle-box text-center diskbox">' +
                ...
                '<div class="right"></div>' +
                '</div>' +
                '<div class="mask" style="color:' + LoadColor + '" data="Informasi inode<br>Total：' + rdata[i].inodes[0] + '<br>Digunakan: ' + rdata[i].inodes[1] + '<br>Tersedia: ' + rdata[i].inodes[2] + '<br>Penggunaan inode：' + rdata[i].inodes[3] + '"><span>' + rdata[i].size[3].replace('%', '') + '</span>%</div>' +
                '</div>' +
...
function clearSystem() {
    ...
    $.get('/system?action=ClearSystem', function(rdata) {
        layer.close(loadT);
        layer.msg('The cleaning is completed, a total of [' + rdata[0] + '] files will be cleaned up, and [' + toSize(rdata[1]) + '] disk space will be released!', { icon: 1 });
    });
}
...
function NetImg() {
    ...
    // Displays the chart using the configuration items and data just specified.
...
setTimeout(function() {
    $.get('/system/update_server?type=check', function(rdata) {
        ...
        if (rdata.data != undefined) {
            $("#toUpdate").html('<a class="btlink" href="javascript:updateMsg();">Renew X</a>');
...
function updateVersion(version) {
    var loadT = layer.msg('Upgrading panel..', { icon: 16, time: 0, shade: [0.3, '#000'] });
...
function reBoot() {
    ...
    $('.rebt-con a').click(function () {
        ...
        switch (type) {
            case 'panel':
                layer.confirm('About to restart the panel service, continue?？', { title: 'Restart panel service', closeBtn: 2, icon: 3 }, function () {
                ...
                var rebootbox = layer.open({
                    ...
                    content: "<div class='bt-form bt-window-restart'>\
                            ...
                            <div class='SafeRestart' style='line-height:26px'>\
                                <p>Safe restart is good for keeping files safe and will do the following:</p>\
                                <p>1. Stop Web service</p>\
                            ...
                            <div class='bt-form-submit-btn'>\
                                ...
                                <button type='button' class='btn btn-success btn-sm WSafeRestart' >Yes</button>\
                            </div>\
                        </div>"
                });
                setTimeout(function () {
                    ...
                    $(".WSafeRestart").click(function () {
                        ...
                        pluginIndexService('openresty', 'stop', function (r1) {
                            ...
                            $(".SafeRestartCode").append("<p>Stopping MySQL service...</p>");
                                ...
                                $.post('/system/restart_server', '',function (rdata) {
                                    ...
                                    var sEver = setInterval(function () {
                                       $.get("/system/system_total", function(info) {
                                            clearInterval(sEver);
                                            $(".SafeRestartCode p").addClass('c9');
                                            $(".SafeRestartCode").append("<p>Server restarted successfully!...</p>");
                                            setTimeout(function () {
                                                layer.closeAll();
                                            }, 3000);
                                        })
...
function showDanger(num, port) {
    var atxt = "Because no security isolation is used to log in, all IPs can try to connect, there is a high risk, please deal with it immediately.";
    if (port == "22") {
        atxt = "Because the default 22 port of SSH has not been modified, and security isolation is not used to log in, all IPs can try to connect. There is a high risk, please deal with it immediately.";
    }
    layer.open({
        type: 1,
        area: ['720px', '410px'],
        ...
        content: '<div class="pd20">\
				...
				<tr><td class="text-right" width="150">Risk Type: </td><td class="f16" style="color:red">Brute force <a href="http://slemp.basoro.id/docs/bruce-attack" class="btlink f14" style="margin-left:10px" target="_blank">illustrate</a></td></tr>\
				<tr><td class="text-right">Attacks encountered: </td><td class="f16" style="color:red">' + num + ' <a href="javascript:showDangerIP();" class="btlink f14" style="margin-left:10px">detailed</a><span class="c9 f12" style="margin-left:10px">（The data comes directly from this server log）</span></td></tr>\
				...
				<tr><td class="text-right" style="vertical-align:top">Reference solutions: </td><td><p style="margin-bottom:8px">Solution 1: Modify the default port of SSH, modify the SSH authentication method to digital certificate, and clear the recent login log.</p><p>Solution 2: Purchase the Enterprise Operation and Maintenance Edition, and deploy the security isolation service with one click, which is efficient and convenient.</p></td></tr>\
				</tbody></table>\
				<div class="mtb20 text-center"><a href="http://slemp.basoro.id/admin/index.html" target="_blank" class="btn btn-success">Deploy quarantine protection now</a></div>\
				</div>'
    });
    $(".showDanger td").css("padding", "8px")
}

//Total number of key data loaded
loadKeyDataCount();
