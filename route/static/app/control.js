//Displays a 7-day period chart by default
setTimeout(function(){
	Wday(0,'getload');
},500);
setTimeout(function(){
...
function closeControl(){
	layer.confirm('Did you really clear all monitoring records？',{title:'Clear record',icon:3,closeBtn:2}, function() {
		loadT = layer.msg('Processing, please wait...',{icon:16,time:0})
...
function disk(b,e){
	$.get('/system/get_disk_io?start='+b+'&end='+e,function(rdata){
		...
		option = {
			...
			legend: {
				data:['Number of bytes read','Number of bytes written']
			},
			xAxis: {
				type: 'category',
				boundaryGap: false,
	...
	$.get('/system/get_load_average?start='+b+'&end='+e,function(rdata){
		...
		option = {
			...
			xAxis: [
				...
				{ // Cartesian coordinate system grid's x-axis
					type: 'category',
					axisLine: {
						lineStyle: {
							color: '#666'
