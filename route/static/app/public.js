	...
	var c = layer.open({
		...
		<div class='path-con-right'><ul class='default' id='computerDefautl'></ul><div class='file-list divtable'>\
			<table class='table table-hover' style='border:0 none'>\
				<thead><tr class='file-list-head'><th width='40%'>File name</th><th width='20%'>Updated</th><th width='10%'>Permission</th><th width='10%'>Owner</th><th width='10%'></th></tr></thead>\
				...
				<button type='button' class='btn btn-danger btn-sm mr5' onclick=\"layer.close(getCookie('ChangePath'))\">Close</button>\
				<button type='button' class='btn btn-success btn-sm' onclick='getfilePath()'>Choose</button>\
		</div>"
	});
...
$("#dologin").click(function() {
	layer.confirm('Do you really want to exit the panel?', {icon:3,closeBtn: 2}, function() {
			...
			switch(g.data[d].status) {
				case "-1":
					...
					if(g.data[d].type != "download") {
						b = "<li><span class='titlename'>" + g.data[d].name + "</span><span class='state'>Installing <img src='/static/img/ing.gif'> | <a href=\"javascript:removeTask(" + g.data[d].id + ")\">Close</a></span><span class='opencmd'></span><pre class='cmd'></pre></li>"
					} else {
	...
	$.post("/config/get_panel_list",function(rdata){
		...
		$(".mypcipnew").hover(function(){
			...
			layer.open({
			  type: 2,
			  title: false,
			  closeBtn: 0, //Don't show close button
			  shade: [0],
			  area: ['340px', '215px'],
			  offset: 'rb', //pop-up
			  time: 5, //Automatically shuts off after 2 seconds
			  anim: 2,
			...
			layer.msg('Panel opening...',{icon:16,shade: [0.3, '#000'],time:1000});
			setTimeout(function(){
				$("#toBtpanel").submit();
			},500);
			setTimeout(function(){
...
//Add panel quick login
function bindPanel(a,type,ip,btid,url,user,pw){
	var titleName = 'Association panel';
	if(type == "b"){
		btn = "<button type='button' class='btn btn-success btn-sm' onclick=\"bindPanel(1,'b')\">Add</button>";
	} else {
		titleName = 'Modify association ' + ip;
		btn = "<button type='button' class='btn btn-default btn-sm' onclick=\"bindPaneldel('"+btid+"')\">Delete</button><button type='button' class='btn btn-success btn-sm' onclick=\"bindPanel(1,'c','"+ip+"','"+btid+"')\" style='margin-left:7px'>Edit</button>";
	}
	...
	if(a == 1) {
		...
		if(!btaddress.match(/^(http|https)+:\/\/([\w-]+\.)+[\w-]+:\d+/)){
			layer.msg('Panel address format is incorrect, example: <p>http://192.168.0.1:7200</p>',{icon:5,time:5000});
			return;
		}
	...
	layer.open({
		...
		content: "<div class='bt-form pd20 pb70'>\
				...
				<div class='line'><ul class='help-info-text c7'>\
					<li>Favorite other server panel information, realize one-key login panel function</li><li>Panel comments cannot be repeated</li>\
					<li><font style='color:red'>Note that turning on ad blocking will make it impossible to log in quickly.</font></li></ul>\
				</div>\
				<div class='bt-form-submit-btn'><button type='button' class='btn btn-danger btn-sm' onclick=\"layer.closeAll()\">Close</button> "+btn+"</div>\
			</div>"
	});
				...
				if(h.task[g].status == "-1") {
					if(h.task[g].type != "download") {
						...
						if(h.task[g].name.indexOf("scanning") != -1) {
							b = "<li><span class='titlename'>" + h.task[g].name + "</span><span class='state'>Scanning <img src='/static/img/ing.gif'> | <a href=\"javascript:removeTask(" + h.task[g].id + ")\">Close</a></span><span class='opencmd'></span><div class='cmd'>" + c + "</div></li>"
						} else {
							b = "<li><span class='titlename'>" + h.task[g].name + "</span><span class='state'>Installing <img src='/static/img/ing.gif'> | <a href=\"javascript:removeTask(" + h.task[g].id + ")\">Close</a></span><div class='cmd'>" + c + "</div></li>"
						}
					} else {
...
function tasklist(a){
	var con='<ul class="cmdlist"></ul>\
		<span style="position:  fixed;bottom: 13px;">If the task has not been executed for a long time, please try to reset the task queue by clicking [Restart Panel] on the home page</span>';
	$(".taskcon").html(con);
	a = a == undefined ? 1 : a;
	$.post("/task/list", "tojs=getTaskList&table=tasks&limit=10&p=" + a, function(g) {
		var e = "";
		...
		for(var d = 0; d < g.data.length; d++) {
			switch(g.data[d].status) {
				case "-1":
					f = true;
					if(g.data[d].type != "download") {
						b = "<li><span class='titlename'>" + g.data[d].name + "</span><span class='state pull-right c6'>Installing <img src='/static/img/ing.gif'> | <a class='btlink' href=\"javascript:removeTask(" + g.data[d].id + ")\">Close</a></span><span class='opencmd'></span><pre class='cmd'></pre></li>"
					} else {
						b = "<li><div class='line-progress' style='width:0%'></div><span class='titlename'>" + g.data[d].name + "<a id='speed' style='margin-left:130px;'>0.0M/12.5M</a></span><span class='com-progress'>0%</span><span class='state'>Downloading <img src='/static/img/ing.gif'> | <a href=\"javascript:removeTask(" + g.data[d].id + ")\">Close</a></span></li>"
					}
					task_count++;
					break;
				case "0":
					c += "<li><span class='titlename'>" + g.data[d].name + "</span><span class='state pull-right c6'>Wait</span> | <a href=\"javascript:removeTask(" + g.data[d].id + ")\" class='btlink'>"+lan.public.del+"</a></li>";
					task_count++;
					break;
			}
		}
...
/*** Among them, the function is used by the library for the plug-in start ***/
...
function pluginOpService(a, b, v) {
    ...
    layer.confirm( msgTpl('Do you really want {1}{2}{3} services？', [d,a,v]), {icon:3,closeBtn: 2}, function() {
        ...
        $.post("/plugins/run", c, function(g) {
            ...
            layer.close(e);
            layer.msg('Abnormal operation!', {icon: 1});
        });
    })
}

//Configuration modification --- start
function pluginConfig(_name, version, func){
    ...
    var con = '<p style="color: #666; margin-bottom: 7px">Tip: Ctrl+F to search keywords, Ctrl+G to find next, Ctrl+S to save, Ctrl+Shift+R to find and replace!</p>\
                ...
                <ul class="help-info-text c7 ptb15">\
                    <li>Here is the '+ _name + version +' main configuration file. If you do not understand the configuration rules, please do not modify it at will.</li>\
                </ul>';
    $(".soft-man-con").html(con);

    var loadT = layer.msg('Configuration file path is getting...',{icon:16,time:0,shade: [0.3, '#000']});
    $.post('/plugins/run', {name:_name, func:func_name,version:version},function (data) {
        layer.close(loadT);

        var loadT2 = layer.msg('File content is getting...',{icon:16,time:0,shade: [0.3, '#000']});
...
//Configuration modification template --- start
function pluginConfigTpl(_name, version, func, config_tpl_func, read_config_tpl_func){
    ...
    $.post('/plugins/run',{name:_name, func:_config_tpl_func,version:version}, function(data){
    	...
    	$('#config_tpl').change(function(){
    		...
    		if (selected != '0'){
    			var loadT = layer.msg('Configuration template is getting...',{icon:16,time:0,shade: [0.3, '#000']});
...
function pluginOpInitD(a, _version, b) {
    ...
    layer.confirm( msgTpl('Do you really want {1}{2}{3} services?', [d,a,_version]), {icon:3,closeBtn: 2}, function() {
...
function pluginLogs(_name, version, func, line){
    ...
    var loadT = layer.msg('Getting log path...',{icon:16,time:0,shade: [0.3, '#000']});
    $.post('/plugins/run', {name:_name, func:func_name, version:version},function (data) {
        layer.close(loadT);
        ...
        $.post('/files/get_last_body', 'path=' + fileName+'&line='+file_line, function(rdata) {
            ...
            if(rdata.data == '') {
            	rdata.data = 'There are currently no logs!';
            }
...
/*** Among them, the function is used by the library for the plug-in end ***/
