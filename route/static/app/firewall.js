...
$("#firewalldType").change(function(){
	...
	var p = 'Port';
	var t = 'Release';
	var m = 'Description: Support release port range, such as: 3000:3500';
	if(type == 'address'){
		w = '150px';
		p = 'IP address to be blocked';
		t = 'Block';
		m = 'Description: Supports masking of IP segments, such as: 192.168.0.0/24';
	}
...
/**
 * Change ping ban status
 * @param {Int} state 0.ban ping 1.pingable
 */
 ...
 * @param {Int} state 0,turn on 1.disabled
 */
...
/**
 * Set remote service status
 * @param {Int} state 0.enable 1.close
 */
function setMstscStatus(){
	status = $("#sshswitch").prop("checked")==true?1:0;
	var msg = status==1?'Disabling the SSH service will also log out all logged in users. Continue? ':' Are you sure you want to enable SSH service?';
		...
		for (var i = 0; i < data.data.length; i++) {
			...
			switch(data.data[i].status){
				case 0:
					...
					break;
				case 1:
					status = 'No access to external network';
					break;
				default:
					status = 'Normal';
					break;
...
//Clean up panel logs
function delLogs(){
	layer.confirm('About to clear the panel log, continue?',{title:'Clear log',closeBtn:2},function(){
