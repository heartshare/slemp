	...
	$.post("/crontab/list?p="+page,'', function(rdata){
		...
		if(rdata == ""){
			...
			for(var i=0;i<rdata.data.length;i++){
				...
				'<span class="btOpen" onclick="setTaskStatus(' + rdata.data[i].id + ',0)" style="color:rgb(92, 184, 92);cursor:pointer" title="Disable this scheduled task">Normal<span class="glyphicon glyphicon-play"></span></span>'
				:'<span onclick="setTaskStatus('+ rdata.data[i].id +',1)" class="btClose" style="color:red;cursor:pointer" title="Enable this scheduled task">Disable<span style="color:rgb(255, 0, 0);" class="glyphicon glyphicon-pause"></span></span>';
				...
				cbody += "<tr>\
							...
							<td>\
								...
								<a href=\"javascript:planDel("+rdata.data[i].id+" ,'"+rdata.data[i].name.replace('\\','\\\\').replace("'","\\'").replace('"','')+"');\" class='btlink'>Delete</a>\
							</td>\
						</tr>";
			}
		}
...
function setTaskStatus(id,status){
	var confirm = layer.confirm(status == '0'?'A scheduled task cannot continue to run after it is paused. Do you really want to deactivate this scheduled task?':'The scheduled task is disabled, do you want to enable this scheduled task', {title:'Hint',icon:3,closeBtn:2},function(index) {
		if (index > 0) {
...
//Simulate synchronization to start batch delete database
			...
			layer.open({
				...
				content:'<div class="setting-con ptb20">\
								...
								<div class="clearfix plan ptb10 site_list" style="display:none">\
									...
									<div class="textname pull-left mr20">Backup to</div>\
										...
										<div class="plan_hms pull-left mr20 bt-input-text">\
											<span><input type="number" name="save" class="save_create" value="'+ obj.from.save +'" maxlength="4" max="100" min="1"></span><span class="name">Share</span>\
										</div>\
									</div>\
								</div>\
								...
								<div class="clearfix plan ptb10" style="display:'+ (obj.from.stype == "rememory"?'block;':'none') +'">\
									...
									<div style="line-height:34px">Release the memory usage of PHP, MYSQL, NGINX, and it is recommended to execute it in the middle of the night every day!</div>\
								</div>\
