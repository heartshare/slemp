...
class backupTools:
    ...
    def backupSite(self, name, count):
        ...
        if not path:
            ...
            log = u"website [" + name + "] does not exist!"
            print(u"★[" + endDate + "] " + log)
            print(
                "----------------------------------------------------------------------------")
            return
        ...
        if not os.path.exists(filename):
            log = u"website [" + name + u"] backup failed!"
            print(u"★[" + endDate + "] " + log)
            print(
                u"----------------------------------------------------------------------------")
            return
        ...
        log = u"website [" + name + u"] backup succeeded, time consumed [" + str(round(outTime, 2)) + u"] second"
        slemp.writeLog(u'Scheduled Tasks', log)
        print(u"★[" + endDate + "] " + log)
        print(u"|---Keep the latest [" + count + u"] backup")
        ...
        if num > 0:
            for backup in backups:
                ...
                num -= 1
                print(u"|---Expired backup files have been cleaned up: " + backup['filename'])
                if num < 1:
                    break
    ...
    def backupDatabase(self, name, count):
        ...
        if not name:
            endDate = time.strftime('%Y/%m/%d %X', time.localtime())
            log = u"database [" + name + u"] does not exist!"
            print(u"★[" + endDate + "] " + log)
            print(
                u"----------------------------------------------------------------------------")
            return
        ...
        if not os.path.exists(filename):
            endDate = time.strftime('%Y/%m/%d %X', time.localtime())
            log = u"database [" + name + u"] backup failed!"
            print(u"★[" + endDate + "] " + log)
            print(
                u"----------------------------------------------------------------------------")
            return
        ...
        slemp.M('backup').add('type,name,pid,filename,addtime,size', (1, os.path.basename(
            filename), pid, filename, endDate, os.path.getsize(filename)))
        log = u"database [" + name + u"] backup succeeded, time consumtion [" + str(round(outTime, 2)) + u"] second"
        slemp.writeLog(u'Scheduled Tasks', log)
        print("★[" + endDate + "] " + log)
        print(u"|---keep the latest [" + count + u"] backup")
        print(u"|---Filename: " + filename)
