...
function phpSetConfig(version) {
    phpPost('get_php_conf', version,'',function(data){
        ...
        for (var i = 0; i < rdata.length; i++) {
            ...
            switch (rdata[i].type) {
                case 0:
                    ...
                    ibody = '<select class="bt-input-text mr5" name="' + rdata[i].name + '" style="width: ' + w + 'px;"><option value="1" ' + selected_1 + '>Stop</option><option value="0" ' + selected_0 + '>Stop</option></select>'
                    break;
                case 1:
        ...
        var phpCon = '<style>.conf_p p{margin-bottom: 2px}</style><div class="conf_p" style="margin-bottom:0">\
                        ' + mlist + '\
                        <div style="margin-top:10px; padding-right:15px" class="text-right"><button class="btn btn-success btn-sm mr5" onclick="phpSetConfig(' + version + ')">Reload</button><button class="btn btn-success btn-sm" onclick="submitConf(' + version + ')">Save</button></div>\
                    </div>'
...
function getFpmConfig(version){
    phpPost('get_fpm_conf', version, {}, function(data){
        ...
        var body = "<div class='bingfa'>" +
            ...
            "<p class='line'><span class='span_tit'>Operation mode: </span><select class='bt-input-text' name='pm' style='width:100px;'>" + pmList + "</select><span class='c9'>* PHP-FPM operating mode</span></p>" +
            ...
            "<p class='line'><span class='span_tit'>start_servers：</span><input class='bt-input-text' type='number' name='start_servers' value='" + rdata.start_servers + "' />  <span class='c9'>* Number of starting processes (the number of initial processes after the service starts)</span></p>" +
...
function getFpmStatus(version){
    phpPost('get_fpm_status', version, '', function(ret_data){
        ...
        var con = "<div style='height:420px;overflow:hidden;'><table class='table table-hover table-bordered GetPHPStatus' style='margin:0;padding:0'>\
                        ...
                        <tr><th>Process management method (process manager)</th><td>" + ((rdata['process manager'] == 'dynamic') ? 'dynamic' : 'static') + "</td></tr>\
                        ...
                        <tr><th>Process limit reached (max children reached)</th><td>" + rdata['max children reached'] + "</td></tr>\
...
function disableFunc(version) {
    phpPost('get_disable_func', version,'',function(data){
        ...
        con += '<ul class="help-info-text">\
                    <li>Here you can disable calling of the specified function to enhance environment security!</li>\
...
function uninstallPHPLib(version, name, title, pathinfo) {
    layer.confirm('Do you really want to remove {1}?'.replace('{1}', name), { icon: 3, closeBtn: 2 }, function() {
        name = name.toLowerCase();
