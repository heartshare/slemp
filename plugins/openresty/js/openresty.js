...
function orPluginOpService(a, b, v) {
    ...
    layer.confirm( msgTpl('Do you really want {1}{2}{3} services? ', [d,a,v]), {icon:3,closeBtn: 2}, function() {
        orPost('get_os',{},function(data){
            ...
            if (!rdata['auth']){
                layer.prompt({title: 'Insufficient permission is checked, and a password is required!', formType: 1},function(pwd, index){
    ...
    $.post('/plugins/run', {name:'openresty', func:'run_info'}, function(data) {
        ...
        try {
            ...
            var con = "<div><table class='table table-hover table-bordered'>\
                            <tr><th>Active connection (active)</th><td>" + rdata.active + "</td></tr>\
                            <tr><th>Number connection(accepts)</th><td>" + rdata.accepts + "</td></tr>\
                            <tr><th>Total handshakes(handled)</th><td>" + rdata.handled + "</td></tr>\
                            <tr><th>Total requests(requests)</th><td>" + rdata.requests + "</td></tr>\
                            <tr><th>Number requests(Reading)</th><td>" + rdata.Reading + "</td></tr>\
                            <tr><th>Number responses(Writing)</th><td>" + rdata.Writing + "</td></tr>\
