...
function myDbPos(){
    myPost('my_db_pos','',function(data){
        var con = '<div class="line ">\
            ...
            <button id="btn_change_path" name="btn_change_path" class="btn btn-success btn-sm mr5 ml5 btn_change_port">Migrate</button>\
            </div></div>';
        $(".soft-man-con").html(con);
...
function myPort(){
    myPost('my_port','',function(data){
        var con = '<div class="line ">\
            ...
            <button id="btn_change_port" name="btn_change_port" class="btn btn-success btn-sm mr5 ml5 btn_change_port">Revise</button>\
            </div></div>';
        $(".soft-man-con").html(con);
...
function myPerfOpt() {
    ...
    myPost('db_status','',function(data){
        ...
        var memCon = '<div class="conf_p" style="margin-bottom:0">\
                        ...
                        <div style="margin-top:10px; padding-right:15px" class="text-right"><button class="btn btn-success btn-sm mr5" onclick="reBootMySqld()">Restart</button><button class="btn btn-success btn-sm" onclick="setMySQLConf()">Save</button></div>\
                    </div>'
...
//Setting MySQL configuration parameters
...
//MySQL memory optimization solution
...
//Calculate MySQL memory overhead
...
function setRootPwd(type, pwd){
    ...
    var index = layer.open({
        ...
        content: "<form class='bt-form pd20 pb70' id='mod_pwd'>\
                    ...
                    <div class='info-r'><input class='bt-input-text mr5' type='text' name='password' id='MyPassword' style='width:330px' value='"+pwd+"' /><span title='Random code' class='glyphicon glyphicon-repeat cursor' onclick='repeatPwd(16)'></span></div>\
                    </div>\
                    <div class='bt-form-submit-btn'>\
                        <button id='my_mod_close' type='button' class='btn btn-danger btn-sm btn-title'>Close</button>\
                        <button type='button' class='btn btn-success btn-sm btn-title' onclick=\"setRootPwd(1)\" >Submit</button>\
                    </div>\
                  </form>",
    });
    ...
    myPost('get_db_access','username='+username, function(data){
        ...
        var index = layer.open({
            ...
            content: "<form class='bt-form pd20 pb70' id='set_db_access'>\
                        ...
                        <div class='bt-form-submit-btn'>\
                            <button id='my_mod_close' type='button' class='btn btn-danger btn-sm btn-title'>Close</button>\
                            <button id='my_mod_save' type='button' class='btn btn-success btn-sm btn-title'>Submit</button>\
                        </div>\
                      </form>",
        });
...
function setDbPass(id, username, password){
    ...
    var index = layer.open({
        ...
        content: "<form class='bt-form pd20 pb70' id='mod_pwd'>\
                    ...
                    <div class='info-r'><input class='bt-input-text mr5' type='text' name='password' id='MyPassword' style='width:330px' value='"+password+"' /><span title='Random code' class='glyphicon glyphicon-repeat cursor' onclick='repeatPwd(16)'></span></div>\
                    </div>\
...
function addDatabase(type){
    ...
    var index = layer.open({
        ...
        content: "<form class='bt-form pd20 pb70' id='add_db'>\
                    ...
                    <div class='bt-form-submit-btn'>\
                        <button id='my_mod_close' type='button' class='btn btn-danger btn-sm btn-title'>Close</button>\
                        <button type='button' class='btn btn-success btn-sm btn-title' onclick=\"addDatabase(1)\" >Submit</button>\
                    </div>\
                  </form>",
    });
...
function setDbPs(id, name, obj) {
    var _span = $(obj);
    var _input = $("<input class='baktext' value=\""+_span.text()+"\" type='text' placeholder='Remarks' />");
...
function openPhpmyadmin(name,username,password){
    ...
    layer.msg('Opening phpMyAdmin',{icon:16,shade: [0.3, '#000'],time:1000});
...
function dbList(page, search){
    ...
    myPost('get_db_list', _data, function(data){
        ...
        for(i in rdata.data){
            ...
            list += '<a href="javascript:;" class="btlink" class="btlink" onclick="setBackup(\''+rdata.data[i]['name']+'\',this)" title="Database backup">'+(rdata.data[i]['is_backup']?'backup':'not backed up') +'</a> | ';
        ...
        var con = '<div class="safe bgw">\
            ...
            <span style="float:right">              \
                <button batch="true" style="float: right;display: none;margin-left:10px;" onclick="delDbBatch();" title="Delete selected item" class="btn btn-default btn-sm">Delete</button>\
            </span>\
...
function myLogs(){
    ...
    myPost('bin_log', {status:1}, function(data){
        ...
        var limitCon = '<p class="conf_p">\
                        ...
                        <p class="f14 c6 mtb10" style="border-top:#ddd 1px solid; padding:10px 0">Error log<button class="btn btn-default btn-clear btn-xs" style="float:right;" >Empty</button></p>\
...
function selectedTools(my_obj, db_name) {
    ...
    if (n > 0) {
        var my_btns = '<button class="btn btn-default btn-sm" onclick="repDatabase(\'' + db_name + '\',null)">Repair</button>\
            <button class="btn btn-default btn-sm" onclick="optDatabase(\'' + db_name + '\',null)">Optimize</button>\
        ...
        for (var i = 0; i < rdata.tables.length; i++) {
            ...
            tbody += '<tr>\
                    ...
                    <td style="text-align: right;">\
                        ...
                        <a class="btlink" onclick="optDatabase(\''+ db_name + '\',\'' + rdata.tables[i].table_name + '\')">Optimize</a> |\
...
function addMasterRepSlaveUser(){
    ...
    var index = layer.open({
        ...
        content: "<form class='bt-form pd20 pb70' id='add_master'>\
            ...
            <div class='bt-form-submit-btn'>\
                <button id='my_mod_close' type='button' class='btn btn-danger btn-sm btn-title'>Close</button>\
                <button type='button' class='btn btn-success btn-sm btn-title' id='submit_add_master' >Submit</button>\
            </div>\
          </form>",
    });
    ...
    myPost('get_master_rep_slave_list', _data, function(data){
        ...
        for (i in user_list) {
            ...
            list += '<tr><td>'+name+'</td>\
                ...
                <td>\
                    <a class="btlink" onclick="updateMasterRepSlaveUser(\''+name+'\');">Revise</a> | \
        ...
        page += '<div class="table_toolbar"><span class="sync btn btn-default btn-sm" onclick="addMasterRepSlaveUser()" title="">Add sync account</span></div>';
            ...
            content:"<div class='bt-form pd20 c6'>\
                     <div class='divtable mtb10'>\
                        <div><table class='table table-hover'>\
                            <thead><tr><th>Users</th><th>Password</th><th>Action</th></tr></thead>\
                            <tbody>" + list + "</tbody>\
                        </table></div>\
                        "+page +"\
                    </div>\
...
function getFullSyncStatus(db){
    ...
    var btn = '<div class="table_toolbar"><span class="sync btn btn-default btn-sm" id="begin_full_sync" title="">Start</span></div>';
    var loadOpen = layer.open({
        type: 1,
        title: 'Full sync ['+db+']',
        area: '500px',
...
function handlerRun(){
    cmd = 'cd /home/slemp/server/panel && python /home/slemp/server/panel/plugins/mysql/index.py do_full_sync {"db":"all"}';
    var loadOpen = layer.open({
        type: 1,
    ...
    function getMasterDbList(){
        ...
        myPost('get_masterdb_list', _data, function(data){
            ...
            var con = '<div class="divtable mtb10">\
                    ...
                    <div id="databasePage" class="dataTables_paginate paging_bootstrap page"></div>\
                    <div class="table_toolbar">\
                        <span class="sync btn btn-default btn-sm" onclick="getMasterRepSlaveList()" title="">Sync Account List</span>\
                    </div>\
                </div>';
        ...
        myPost('get_masterdb_list', _data, function(data){
            ...
            for(i in rdata.data){
                ...
                list += '<td style="text-align:right">' +
                    ...
                    '<a href="javascript:;" class="btlink" onclick="getFullSyncStatus(\''+rdata.data[i]['name']+'\')" title="Repair">Repair</a>' +
                '</td>';
                list += '</tr>';
            }
    ...
    function getMasterStatus(){
        myPost('get_master_status', '', function(data){
            ...
            var limitCon = '<p class="conf_p">\
                    ...
                    <button class="btn '+(!rdata.status ? 'btn-danger' : 'btn-success')+' btn-xs btn-master va0">'+(!rdata.status ? 'Unopened' : 'Activated') +'</button><hr/>\
                </p>\
                <!-- master list -->\
                <div class="safe bgw table_master_list"></div>\
                <hr/>\
                <p class="conf_p">\
                    <span class="f14 c6 mr20">Slave [slave] Configuration</span><span class="f14 c6 mr20"></span>\
                    <button class="btn '+(!rdata.data.slave_status ? 'btn-danger' : 'btn-success')+' btn-xs btn-slave va0">'+(!rdata.data.slave_status ? 'Have not started' : 'Activated') +'</button><hr/>\
                </p>\
                <!-- slave status list -->\
